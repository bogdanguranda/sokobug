...
public abstract class LabyrinthObject {
	...
	public enum Type {
		...
		private Type(String associatedString) {
			...
			if (associatedString.compareTo("B") == 0 || associatedString.compareTo("P") == 0
					|| associatedString.compareTo("W") == 0) {
	...
	public LabyrinthObject isCollidingWith(LabyrinthObject otherObject) {
		...
		if (otherObject.isCollidable()) {
			if (otherObject.getPositionLine() == getPositionLine()
					&& otherObject.getPositionColumn() == getPositionColumn()) {
				return this;
			}
		}
		return null;
