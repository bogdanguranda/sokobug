...
public class AnimationMovingObject extends MovingObject {
	...
	public void draw(SpriteBatch batch) {
		if (getOrientation() == MoveDirection.LEFT) {
			batch.draw(animation.getKeyFrame(stateTime), positionX, positionY, animation.getKeyFrame(stateTime)
					.getRegionWidth() / 2.f, animation.getKeyFrame(stateTime).getRegionHeight() / 2.f, animation
					.getKeyFrame(stateTime).getRegionWidth(), animation.getKeyFrame(stateTime).getRegionHeight(), 1.f,
					1.f, 90.f);
